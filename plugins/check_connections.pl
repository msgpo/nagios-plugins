...
my $data_output = "Established connections: ".$count;
my $perfdata_output = "connections=$count;$opt_w;$opt_c;0;1024";

if ($count >= $opt_c) {
	print "<b>CRITICAL: ".$data_output."</b>|".$perfdata_output."\n";
	$exit_mode = $ERRORS{"CRITICAL"};
} elsif( $count >= $opt_w) {
	print "<b>WARNING: ".$data_output."</b>|".$perfdata_output."\n";
	$exit_mode = $ERRORS{"WARNING"};
} else {
	print "OK: ".$data_output."|".$perfdata_output."\n";
	$exit_mode = $ERRORS{"OK"};
}
